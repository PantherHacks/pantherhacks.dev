...
const EventCard: React.FC<CalendarEvent> = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col p-[2px] w-72 sm:w-[35rem] md:w-[45rem]",
        startTimestamp.getTime() <= Date.now() && endTimestamp.getTime() >= Date.now() && " highlighted-card"
      )}
    >
      <div
        className={cn(
      ...
      >
        <div className="flex flex-col md:flex-row items-start md:items-center gap-2 mb-2 md:mb-0">
          ...
          <Badge
            className={cn(
              "h-6 flex-shrink-0 cursor-default whitespace-nowrap",
              getCalendarEventColors(activityType)[0]
            )}
          >
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-y-2 sm:gap-y-0">
          ...
          {startTimestamp.getTime() <= Date.now() && endTimestamp.getTime() >= Date.now() ? (
            <h2 className="font-TangoSansBold text-3xl text-[#fd021d] flex flex-row justify-center items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="neon-text-small transform-gpu">LIVE</span>
              </div>
            </h2>
          ) : (
            <Button
